...
char* get_auth_header(char* buf, int buf_len, int connfd)
{
    ...
    {        
        ...
        //If no line is found check if there is more to read
        else
        {
            if(strlen(last_line)<=1)
            {
            ...
            //Copy whatever is after the last line to the beginning of the buffer
            strncpy(buf, last_line+strlen(last_line)+1, buf_len);
            int len_remainder = strlen(buf);
            int r_len = recv(connfd, buf+len_remainder, buf_len-len_remainder,0);
            if(r_len == 0)
            {
                return NULL;
            }
